...
export default function TradeTicket({ instrument, open, onOpenChange, onOrderPlaced }: Props) {
  ...
  const resetForm = () => {
    setSide("Buy"); setOrderType("Market"); setQuantity("1"); setLeverage("100");
    setLimitPrice(""); setStopLoss(""); setTakeProfit(""); setSuccess(false);
  };
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      const entryPrice = orderType === "Market" ? price : parseFloat(limitPrice) || price;

      saveOrder({
        id: orderId,
        symbol: inst.symbol,
        instrumentName: inst.name,
        side, type: orderType, quantity: qty, leverage: lev,
        entryPrice,
      ...
      if (orderType === "Market") {
        savePosition({
          ...
          symbol: inst.symbol,
          instrumentName: inst.name,
          side, size: qty, entryPrice, currentPrice: entryPrice,
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md bg-card border-border/50 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">{inst.name} · Simulated Order Entry</p>
        </DialogHeader>

        {success ? (
          <div className="py-8 text-center space-y-3">
            <CheckCircle2 className="h-10 w-10 mx-auto text-success" />
            <p className="text-sm font-medium text-foreground">Simulated Order Placed</p>
            <p className="text-xs text-muted-foreground font-mono">
              {side} {qty} lot(s) of {inst.symbol} at {price.toFixed(inst.pipSize < 0.001 ? 4 : 2)}
            </p>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-center rounded-lg bg-muted/30 p-3">
              <div>
                ...
                <p className="font-mono text-sm text-foreground">{inst.bid.toFixed(inst.pipSize < 0.001 ? 4 : 2)}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Ask</p>
                <p className="font-mono text-sm text-foreground">{inst.ask.toFixed(inst.pipSize < 0.001 ? 4 : 2)}</p>
              </div>
            </div>
            ...
            {orderType === "Limit" && (
              <div className="space-y-1.5">
                ...
                <Input type="number" value={limitPrice} onChange={(e) => setLimitPrice(e.target.value)}
                  className="bg-muted/30 border-border/50 font-mono" placeholder={price.toString()} />
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={leverage} onValueChange={setLeverage}>
                  <SelectTrigger className="bg-muted/30 border-border/50 font-mono"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {availableLeverages.map((l) => (
                      <SelectItem key={l} value={l.toString()}>1:{l}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" value={stopLoss} onChange={(e) => setStopLoss(e.target.value)}
                  className="bg-muted/30 border-border/50 font-mono" placeholder="Optional" />
              </div>
              <div className="space-y-1.5">
            ...
            <p className="text-[10px] text-muted-foreground leading-relaxed">
              This is a simulated order. No real funds or market execution is involved. Leveraged CFD trading carries significant risk.
            </p>
