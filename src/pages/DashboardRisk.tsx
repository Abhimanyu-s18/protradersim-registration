...
import { ShieldCheck, AlertTriangle, Briefcase, BarChart3 } from 'lucide-react';
import {
  Instrument,
  Position,
  getInstruments,
...
export default function DashboardRisk() {
  ...
  const enriched: EnrichedPosition[] = openPositions.map((p) => {
    const inst = instruments.find((i) => i.symbol === p.symbol);
    if (!inst) {
      // Return with default values if instrument not found
      return { ...p, currentPrice: p.currentPrice, pnl: p.pnl, notional: 0 };
    }
  ...
  const marginLevelCapped =
    m.marginLevel === Infinity ? 100 : Math.min(m.marginLevel, 500);
  const marginBarPct = (marginLevelCapped / 500) * 100;

  // Concentration: highest single exposure as % of total
  const exposureEntries = Object.entries(m.exposureByClass).sort(
    (a, b) => b[1] - a[1]
  );

  // Calculate notional for largest position to determine concentration
  const largestPositionNotional = m.largestPosition
    ? (() => {
        const pos = m.largestPosition;
        const inst = instruments.find((i) => i.symbol === pos.symbol);
        if (!inst) return 0;
        const currentPrice = pos.side === 'Buy' ? inst.bid : inst.ask;
        return currentPrice * pos.size * inst.lotSize;
      })()
    : 0;

  const concentrationPct =
    m.totalExposure > 0 && m.largestPosition
      ? (largestPositionNotional / m.totalExposure) * 100
      : 0;
  ...
  return (
    <DashboardShell title="Risk & Exposure" activeItem="Risk">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          <div key={c.label} className="glass-card rounded-xl p-5">
            ...
            <p
              className={`mt-1 text-xl font-bold font-mono ${c.color || 'text-foreground'}`}
            >
              {c.value}
            </p>
          </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="glass-card rounded-xl p-5 space-y-3">
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
            <BarChart3 className="h-4 w-4 text-primary" /> Exposure by Asset
            Class
          </h3>
        ...
        <div className="glass-card rounded-xl p-5 space-y-3">
          <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 text-primary" /> Concentration &
            Warnings
          </h3>
          <div className="space-y-2">
              ...
              <>
                {concentrationPct > 50 && (
                  <div className="rounded-lg bg-primary/10 border border-primary/20 px-3 py-2 text-xs text-primary">
                    <span className="font-semibold">High Concentration:</span>{' '}
                    Largest position represents {concentrationPct.toFixed(1)}%
                    of total exposure
                  </div>
                )}
                {m.marginWarning === 'critical' && (
                  <div className="rounded-lg bg-destructive/10 border border-destructive/20 px-3 py-2 text-xs text-destructive">
                    ...
                    Free margin is critically low. Consider closing positions.
                  </div>
                )}
                {m.freeMargin < 0 && (
                  <div className="rounded-lg bg-destructive/10 border border-destructive/20 px-3 py-2 text-xs text-destructive">
                    <span className="font-semibold">Negative Free Margin:</span>{' '}
                    Account is in margin deficit of{' '}
                    {fmt(Math.abs(m.freeMargin))}
                  </div>
                )}
                {openPositions.length > 5 && (
                  <div className="rounded-lg bg-muted/30 border border-border/50 px-3 py-2 text-xs text-muted-foreground">
                    ...
                    {openPositions.length} open positions — monitor total
                    exposure
                  </div>
                )}
                {m.marginWarning === 'none' &&
                  ...
                  openPositions.length <= 5 && (
                    <div className="rounded-lg bg-success/10 border border-success/20 px-3 py-2 text-xs text-success">
                      No active warnings. Account health is within normal
                      parameters.
                    </div>
                  )}
              </>
            ...
            <div className="mt-3 pt-3 border-t border-border/30">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-mono text-foreground">
                  {m.largestPosition.symbol}
                </span>
                <span
                  className={`text-sm font-mono font-medium ${m.largestPosition.pnl >= 0 ? 'text-success' : 'text-destructive'}`}
                >
                  {m.largestPosition.pnl >= 0 ? '+' : ''}
                  {fmt(m.largestPosition.pnl)}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {enriched.map((p) => {
                  const pctOfEquity =
                    m.equity > 0 ? (Math.abs(p.pnl) / m.equity) * 100 : 0;
                  return (
                    <tr
                      key={p.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs font-semibold ${p.side === 'Buy' ? 'text-success' : 'text-destructive'}`}
                        >
                          {p.side}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`font-mono font-medium ${p.pnl >= 0 ? 'text-success' : 'text-destructive'}`}
                        >
                          {p.pnl >= 0 ? '+' : ''}
                          {fmt(p.pnl)}
                        </span>
