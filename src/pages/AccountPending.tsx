...
import { TrendingUp, ArrowLeft, CheckCircle2, Clock, Mail, Shield, Loader2 } from "lucide-react";
import { loadRegistration, saveStep4 } from "@/lib/registration-store";

const AccountPending = () => {
  const reg = loadRegistration();
  const [emailSent, setEmailSent] = useState(reg?.step4?.emailVerificationSent ?? false);
  ...
  const checklist = [
    { label: "Registration submitted", done: !!reg?.completed, icon: CheckCircle2 },
    { label: emailSent ? "Verification email sent" : "Email verification pending", done: emailSent, icon: Mail },
      ...
      <div className="relative z-10 w-full max-w-lg px-4">
        ...
        <div className="glass-card rounded-2xl p-8">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-foreground">Your Account Is Pending Activation</h1>
            <p className="mt-2 text-sm text-muted-foreground max-w-sm mx-auto">
              Your registration has been received. Please verify your email address to proceed with account activation.
            </p>
          </div>
          ...
          <div className="mt-8 space-y-3">
            {checklist.map((item, i) => (
              <div key={i} className="flex items-center gap-3 rounded-lg border border-border/50 bg-muted/30 px-4 py-3">
                <item.icon className={`h-4 w-4 shrink-0 ${item.done ? "text-success" : "text-muted-foreground"}`} />
                <span className={`text-sm ${item.done ? "text-foreground" : "text-muted-foreground"}`}>{item.label}</span>
                {item.done && <span className="ml-auto text-[10px] uppercase tracking-wider font-semibold text-success">Done</span>}
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            >
              {sending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Sending…</> : "Resend Verification Email"}
            </Button>
            <Link to="/register/review" className="flex-1">
              <Button variant="outline" className="w-full">Review Submitted Details</Button>
            </Link>
          </div>
