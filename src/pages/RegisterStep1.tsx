...
import { saveStep1, loadRegistration, type Step1Data } from "@/lib/registration-store";
...
const RegisterStep1 = () => {
  ...
  const validateField = (field: keyof Step1Data, value?: string) => {
    ...
    const result = step1Schema.safeParse({ ...partial, phone: partial.phone || undefined, ibCode: partial.ibCode || undefined });
    if (result.success) {
  ...
  const isValid = useMemo(() => {
    const result = step1Schema.safeParse({ ...form, phone: form.phone || undefined, ibCode: form.ibCode || undefined });
    return result.success;
  }, [form]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setTouched({ firstName: true, lastName: true, email: true, dateOfBirth: true, country: true });
    const result = step1Schema.safeParse({ ...form, phone: form.phone || undefined, ibCode: form.ibCode || undefined });
    if (!result.success) {
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        touched.dateOfBirth && errors.dateOfBirth && "border-destructive"
                      )}
                      onBlur={() => handleBlur("dateOfBirth")}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {dobDate ? format(dobDate, "PPP") : "Select your date of birth"}
                    </Button>
                  </PopoverTrigger>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="ibCode" className="text-sm text-foreground">
                  IB Referral Code <span className="text-muted-foreground text-xs">(optional)</span>
                </Label>
                <Input
                  id="ibCode"
              ...
              <div className="flex items-start gap-2 rounded-lg border border-border/40 bg-muted/20 p-3">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  Your account remains inactive until email verification is completed.
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              ...
              <div className="mt-6 rounded-lg border border-border/40 bg-muted/20 p-4">
                <p className="text-xs text-muted-foreground leading-relaxed">
                  <span className="font-semibold text-foreground">Risk Notice:</span>{" "}
                  ProTraderSim is a simulation platform for educational purposes. No real money is at risk. Past simulated performance does not guarantee future results.
                </p>
              </div>
            </div>
          </aside>
