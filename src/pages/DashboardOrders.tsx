        ...
        <div className="flex gap-2">
          {(["open", "history"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "open" ? `Open Orders (${openOrders.length})` : `Order History (${historyOrders.length})`}
            </button>
          ))}
        </div>
        ...
        <div className="glass-card rounded-xl overflow-hidden">
          {display.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">No {tab === "open" ? "open" : ""} orders yet</p>
              <p className="text-xs text-muted-foreground mt-1">Place a simulated trade from the Markets page</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/50">
                    {["Time", "ID", "Symbol", "Side", "Type", "Qty", "Price", "Status"].map((h) => (
                ...
                <tbody>
                  {display.map((o) => (
                    <tr key={o.id} className="border-b border-border/30 hover:bg-muted/20">
                      <td className="px-4 py-3 text-xs text-muted-foreground font-mono">{new Date(o.timestamp).toLocaleTimeString()}</td>
                      <td className="px-4 py-3 text-xs font-mono text-muted-foreground">{o.id}</td>
                      <td className="px-4 py-3 font-mono font-medium text-foreground">{o.symbol}</td>
                      <td className="px-4 py-3">
                        <span className={`text-xs font-semibold ${o.side === "Buy" ? "text-success" : "text-destructive"}`}>{o.side}</span>
                      </td>
                      <td className="px-4 py-3 text-xs text-muted-foreground">{o.type}</td>
                      <td className="px-4 py-3 font-mono text-foreground">{o.quantity}</td>
                      <td className="px-4 py-3 font-mono text-foreground">{o.entryPrice.toFixed(2)}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[10px] uppercase tracking-wider font-semibold ${
                          o.status === "Filled" ? "text-success" : o.status === "Pending" ? "text-primary" : "text-muted-foreground"
                        }`}>{o.status}</span>
                      </td>
                    </tr>
                  ))}
