...
const DashboardChallenge = () => {
  ...
  const rules = [
    ...
    },
    {
      ...
      progress: Math.min(
        100,
        (challenge.dailyDrawdown / challenge.rules.maxDailyDrawdown) * 100
      ),
    ...
    },
    {
      ...
      progress: Math.min(
        100,
        (challenge.currentDrawdown / challenge.rules.maxTotalDrawdown) * 100
      ),
    ...
    },
    {
      ...
      progress: Math.min(
        100,
        (challenge.tradingDays / challenge.rules.minTradingDays) * 100
      ),
  ...
  const totalDrawdownBuffer = Math.max(
    0,
    challenge.rules.maxTotalDrawdown - challenge.currentDrawdown
  );
  const dailyDrawdownBuffer = Math.max(
    0,
  ...
  return (
    <DashboardShell title="Trader Evaluation" activeItem="Challenge">
      {/* Status Banner */}
      <div
        className={`glass-card rounded-xl p-6 ${sc.bg} border-l-4 ${
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {challenge.status === 'not_started' &&
                'Begin trading to start your simulated evaluation.'}
              {challenge.status === 'in_progress' &&
                'Your evaluation is active. Trade consistently to meet objectives.'}
              {challenge.status === 'passed' &&
                'Congratulations! You have met all simulated challenge objectives.'}
              {challenge.status === 'failed' &&
                `Evaluation breached: ${challenge.breachedRule}. Reset to try again.`}
            </p>
          </div>
        </div>
        {challenge.status !== 'not_started' && (
          <div className="mt-4">
            <div className="flex items-center justify-between text-xs mb-1.5">
              ...
              <span className="font-mono text-foreground">
                {Math.min(100, challenge.progressPercent).toFixed(0)}%
              </span>
            </div>
        ...
        <div className="rounded-xl p-4 flex items-center gap-3 border bg-destructive/10 border-destructive/30">
          ...
          <div>
            ...
            <p className="text-xs text-destructive/80">
              {challenge.breachedRule}. The simulated evaluation has been
              terminated.
            </p>
          </div>
          <Button
            ...
            <div key={rule.label} className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <span
                    className={`font-mono font-semibold ${rule.breached ? 'text-destructive' : rule.ok ? 'text-success' : 'text-foreground'}`}
                  >
                    {rule.current}
                  </span>
                </div>
      ...
      <div className="glass-card rounded-xl p-5">
        ...
        <div className="space-y-2 text-xs text-muted-foreground">
          <p>
            • This is a simulated trader evaluation. No real capital is at risk.
          </p>
          <p>
            • The evaluation assesses your trading discipline across{' '}
            {challenge.rules.minTradingDays}+ trading days.
          </p>
          <p>
            • Drawdown limits are measured against your starting balance of{' '}
            {fmt(account.startingBalance)}.
          </p>
          <p>
            • Consistency in risk management is as important as reaching the
            profit target.
          </p>
          {challenge.status === 'in_progress' && totalDrawdownBuffer < 3 && (
            <p className="text-destructive font-medium">
              ⚠ You are approaching the maximum drawdown limit. Trade
              cautiously.
            </p>
          )}
        </div>
