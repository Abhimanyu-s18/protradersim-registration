...
const RegisterReview = () => {
  ...
  const sections = [
    ...
    step2 && {
      ...
      rows: [
        { label: "Address", value: [step2.addressLine1, step2.addressLine2].filter(Boolean).join(", ") },
    ...
    step3 && {
      ...
      rows: [
        ...
        { label: "Products Traded", value: step3.tradedProducts.join(", ") },
    ...
    <div className="relative min-h-screen bg-background">
      ...
      <main className="relative z-10 container pt-24 pb-16 max-w-2xl">
        <div className="glass-card rounded-2xl p-6 sm:p-8">
          <h1 className="text-2xl font-bold text-foreground mb-1">Submitted Registration</h1>
          <p className="text-sm text-muted-foreground mb-6">
            Reference:{" "}
            <span className="font-mono text-primary">{step4?.referenceId || "—"}</span>
          </p>

          <div className="space-y-4">
            {sections.map((section) => (
              <div key={section.title} className="rounded-lg border border-border/60 bg-muted/20 p-4">
