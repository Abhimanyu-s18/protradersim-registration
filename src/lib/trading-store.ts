...
export function estimatePostTradeMetrics(
  ...
  const newMarginLevel =
    newMarginUsed > 0 ? (newEquity / newMarginUsed) * 100 : Infinity;
  const newExposure = current.totalExposure + newNotional;
  const newExposureByClass = { ...current.exposureByClass };
  newExposureByClass[assetClass] =
    (newExposureByClass[assetClass] || 0) + newNotional;
...
export function getWatchlist(): string[] {
  try {
    ...
    return raw
      ...
      : ['EUR/USD', 'XAU/USD', 'US500', 'BTC/USD', 'AAPL'];
  } catch {
    return [];
  }
}
...
export function generateId(): string {
  return 'SIM-' + crypto.randomUUID().split('-')[0].toUpperCase();
}
