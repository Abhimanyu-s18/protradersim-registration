...
export function calculatePerformanceMetrics(): PerformanceMetrics {
  ...
  const totalLossPnl = Math.abs(
    losses.reduce((s, p) => s + (p.realizedPnl ?? 0), 0)
  );
  const profitFactor =
    totalLossPnl > 0
  ...
  const currentDrawdown =
    currentPeak > 0 ? ((currentPeak - currentEquity) / currentPeak) * 100 : 0;

  const netPnl = account.realizedPnl + unrealizedPnl;
  const totalReturn =
    account.startingBalance > 0 ? (netPnl / account.startingBalance) * 100 : 0;

  // Simple risk-adjusted score: (return / max(drawdown, 1)) capped
  const riskAdjustedScore =
    maxDrawdown > 0
      ? Math.min(10, (totalReturn / maxDrawdown) * 2)
      : totalReturn > 0
        ? 10
        : 0;
...
export function generateInsights(): string[] {
  ...
  if (metrics.winRate >= 60)
    insights.push(
      `Your win rate of ${metrics.winRate.toFixed(0)}% is strong. Maintain your discipline.`
    );
  else if (metrics.winRate < 40 && metrics.totalTrades >= 3)
    insights.push(
      `Win rate is below 40%. Consider refining your entry criteria.`
    );

  // Risk/reward
  if (metrics.avgLoss > metrics.avgWin && metrics.avgWin > 0) {
    insights.push(
      'Average loss exceeds average win. Consider tightening stop-losses or letting winners run longer.'
    );
  }
  if (metrics.profitFactor > 1.5 && metrics.totalTrades >= 3) {
    insights.push(
      `Profit factor of ${metrics.profitFactor.toFixed(2)} suggests a positive edge. Stay consistent.`
    );
  }
  ...
  if (metrics.currentDrawdown > 5) {
    insights.push(
      `Current drawdown at ${metrics.currentDrawdown.toFixed(1)}%. Monitor risk exposure carefully.`
    );
  }
  ...
  if (classPerf.length > 0) {
    ...
    if (worst.pnl < 0)
      insights.push(
        `Largest losses concentrated in ${worst.assetClass}. Review your approach to this market.`
      );
    const best = classPerf.sort((a, b) => b.pnl - a.pnl)[0];
    if (best.pnl > 0)
      insights.push(`Most profitable asset class: ${best.assetClass}.`);
  }
...
export function getChallengeRules(): ChallengeRules {
  try {
    ...
    if (raw) return JSON.parse(raw) as ChallengeRules;
  } catch {
    /* fallthrough */
  }
...
export function evaluateChallenge(): ChallengeState {
  ...
  const todayClosed = positions.filter(
    (p) => p.status === 'Closed' && (p.closedAt ?? 0) >= todayTs
  );
  const todayPnl = todayClosed.reduce((s, p) => s + (p.realizedPnl ?? 0), 0);
  const dailyDrawdown =
    todayPnl < 0 ? Math.abs(todayPnl / account.startingBalance) * 100 : 0;

  // Count unique trading days
  const closedWithDates = positions.filter(
    (p) => p.status === 'Closed' && p.closedAt
  );
  const uniqueDays = new Set(
    closedWithDates.map((p) => new Date(p.closedAt!).toDateString())
  );
  ...
  if (savedStatus === 'in_progress' || savedStatus === 'not_started') {
    ...
    if (
      !breachedRule &&
      currentReturn >= rules.profitTarget &&
  ...
  const progressPercent = Math.min(
    100,
    Math.max(0, (currentReturn / rules.profitTarget) * 100)
  );
...
export function seedLosingTrades() {
  ...
  for (const sym of symbols) {
    ...
    savePosition({
      ...
      leverage: inst.leverageMax,
      openedAt: ts,
      closedAt: ts + 3600000 * Math.random() * 12,
      exitPrice,
      realizedPnl: pnl,
      status: 'Closed',
    });
